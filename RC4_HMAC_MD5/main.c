...
int main(void) {
	...
	unsigned char* encdata = (unsigned char*)malloc(enclen);
	memcpy_s(encdata, enclen, "\x5c\x13\x4c\x89\x52\x60\x44\xdd\xba\x3a\x0a\x37\x64\xd9\xc3\x12" \
	...
	if (result) {
		...
		printf("\nData: ");
		for (int i = HMAC_MD5_LENGTH + CONFOUNDER_LENGTH; i < enclen; ++i) printf("%c", encdata[i]);
		printf("\n");
